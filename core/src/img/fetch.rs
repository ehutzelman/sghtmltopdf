...
impl ImageFetcher {
    ...
    ///
    /// A `/…` reference is resolved as site-root relative from `--base-url`,
    /// so someone who meant a filesystem absolute path would learn nothing
    /// from being shown only the joined path. Name both.
    fn not_found(path: &Path, also_tried: &Option<PathBuf>, error: std::io::Error) -> FetchError {
        match also_tried {
            Some(absolute) => FetchError(format!(
                "{}: {error}\n  \
                 (a reference starting with `/` is resolved as site-root relative to --base-url.\n  \
                 It was also looked up as the absolute path {}, which does not exist either)",
                path.display(),
                absolute.display()
            )),
    ...
    fn read_local(&self, path: &str) -> Result<Vec<u8>, FetchError> {
        ...
        if !self.allowed_dirs.is_empty() {
            ...
            {
                return Err(FetchError(format!(
                    "{}: outside the directories permitted by --allow-path",
                    full_path.display()
                )));
            }
        }
